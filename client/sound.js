...
export class Sound {
  ...
  _envelope(
    ...
    g.setValueAtTime(
      Math.max(0.0001, peak * 0.6),
      start + attack + decay + hold,
    );
    g.linearRampToValueAtTime(
      sustainLevel,
      start + attack + decay + hold + 0.0001,
    );
  ...
  _playNoise({
    ...
    const bufferSize = Math.max(
      1,
      Math.floor(ctx.sampleRate * Math.max(0.02, duration)),
    );
      ...
      case 'hud:feed:update': {
        ...
        for (const it of items) {
          ...
          if (me && it.killer === me) this.sfx.killMine();
            ...
            me &&
            Array.isArray(it.assistIds) &&
            it.assistIds.includes(me)
          ) {
            this.sfx.assist();
          }
        }
