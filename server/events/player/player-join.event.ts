...
export class PlayerJoinedEvent extends BaseEvent<TPlayerJoinEvent> {
  ...
  constructor(private readonly playerId: string, private readonly name: string) { super() }
  ...
  toString(): string {
    return JSON.stringify({ type: this.type, playerId: this.playerId, name: this.name });
  }
}
