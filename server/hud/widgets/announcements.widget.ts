...
import type { SourceEvents, THudAnnouncementsUpdate } from '../../core/types/events.type';
import { HudWidget } from '../types';

export class AnnouncementsWidget implements HudWidget<'announcements'> {
  readonly key = 'announcements';
  private buffer: Array<{ kind: 'streak'; playerId: string; name?: string; category: 'double_kill' | 'killing_spree' | 'unstoppable' | 'rampage' | 'legendary'; streak: number; message: string; timestamp: number }>=[];
  ...
  onEvent(e: SourceEvents): boolean {
    if (e.type === 'streak:changed') {
      ...
      const thresholds: Array<{ n: number, category: 'double_kill' | 'killing_spree' | 'unstoppable' | 'rampage' | 'legendary', message: string }> = [
      ...
      const crossed = thresholds.filter(t => streak >= t.n && prev < t.n).pop();
      if (crossed) {
        const name = (World.players.get(pid)?.name) || pid;
        this.buffer.push({ kind: 'streak', playerId: pid, name, category: crossed.category, streak, message: crossed.message, timestamp: Date.now() });
