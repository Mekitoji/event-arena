import type { SourceEvents, THudFeedUpdate } from '../../core/types/events.type';
import { HudWidget } from '../types';

export class FeedWidget implements HudWidget<'feed'> {
  readonly key = 'feed';
  private buffer: Array<{ killer: string; victim: string; weapon: 'bullet' | 'pellet' | 'rocket' | 'explosion'; assistIds?: string[]; timestamp: number }> = [];
  private max = 8;
  ...
  onEvent(e: SourceEvents): boolean {
    if (e.type === 'feed:entry') {
      const item = { killer: e.killer, victim: e.victim, weapon: e.weapon, assistIds: e.assistIds, timestamp: e.timestamp || Date.now() };
