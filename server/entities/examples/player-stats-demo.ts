...
function demonstratePlayerStats() {
  ...
  player.addKill();
  console.log(
    `   After 1st kill - Streak: ${player.getCurrentStreak()}, Announcement: "${player.getStreakAnnouncement() || 'None'}"`,
  );

  player.addKill();
  console.log(
    `   After 2nd kill - Streak: ${player.getCurrentStreak()}, Announcement: "${player.getStreakAnnouncement() || 'None'}"`,
  );

  player.addKill();
  console.log(
    `   After 3rd kill - Streak: ${player.getCurrentStreak()}, Announcement: "${player.getStreakAnnouncement() || 'None'}"`,
  );

  player.addKill();
  player.addKill();
  console.log(
    `   After 5th kill - Streak: ${player.getCurrentStreak()}, Announcement: "${player.getStreakAnnouncement() || 'None'}"`,
  );
...
function demonstrateStatsManagement() {
  ...
  console.log(
    `   Restored stats - Kills: ${restored.kills}, Streak: ${restored.currentStreak}`,
  );

  // Demonstrate reset
  console.log('\n🔄 Reset demonstration:');
  console.log(
    `   Before reset - Kills: ${stats.kills}, Deaths: ${stats.deaths}`,
  );
  stats.reset();
  console.log(
    `   After reset - Kills: ${stats.kills}, Deaths: ${stats.deaths}`,
  );
}
...
function demonstrateStreakSystem() {
  ...
  for (const targetStreak of streakLevels) {
    ...
    console.log(
      `   ${targetStreak} kills: "${announcement || 'None'}" (${category})`,
    );
  }
}
